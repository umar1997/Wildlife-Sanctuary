<?php
	...
	if($_POST){
		if($_POST['animal_id']){
			...
			if($_POST['feed'] AND $_POST['feed_cost']){
				$query="update `animal` set feed='".$_POST['feed']."',feed_cost='".$_POST['feed_cost']."' where animal_id='".$_POST['animal_id']."'";
				mysqli_query($link,$query);
				$success="Updated successfully";
			}else if($_POST['feed']){
				$query="update `animal` set feed='".$_POST['feed']."' where animal_id='".$_POST['animal_id']."'";
				mysqli_query($link,$query);
				$success="Updated successfully";
			}else if($_POST['feed_cost']){
				$query="update `animal` set feed_cost='".$_POST['feed_cost']."' where animal_id='".$_POST['animal_id']."'";
				mysqli_query($link,$query);
				$success="Updated successfully";
			}
			else if($_POST['feed']=='' AND $_POST['feed_cost']==''){
					$error="Please enter the changes";
			}
			   }else{
				   $error="You didn't enter a valid Animal Id";
			   }
		}else{
			$error="You didn't enter a animal id";
		}
	}
...
<html lang="en">
  ...
  <div class="form-group row">
    <label for="text" class="col-sm-2 col-form-label">Animal ID</label>
    <div class="col-sm-10">
      <input type="text" name="animal_id" class="form-control">
    </div>
  </div>
   <div class="form-group row">
    <label for="text" class="col-sm-2 col-form-label">Feed</label>
    <div class="col-sm-10">
      <input type="text" name="feed" class="form-control">
    </div>
  </div>
   <div class="form-group row">
    <label for="text" class="col-sm-2 col-form-label">Feed Cost</label>
    <div class="col-sm-10">
      <input type="text" name="feed_cost" class="form-control">
    </div>
  </div>
